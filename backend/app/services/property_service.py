...
async def create_property_with_autonomous_units(db: AsyncSession, agency_id: str, prop_in: PropertyCreate) -> Property:
    ...
    if prop_in.type == PropertyType.apartment_complex:
        # floor_config varsa esnek üretim, yoksa eski uniform döngü
        if prop_in.floor_config:
            # Esnek kat yapılandırması: her kat için belirtilen birim sayısı
            calculated_total_units = 0
            door_counter = 1

            for config in sorted(prop_in.floor_config, key=lambda x: x.floor):
                if config.exclude:
                    # Hariç tutulan kat atlanır, kapı numarası artmaz
                    continue
            ...
            if units_to_create:
                db.add_all(units_to_create)
        else:
            # Eski uniform döngü (backward compatible)
    ...
    # 3. Tekil Varlık Ataması (Müstakil / Arsalar / Ticari)
